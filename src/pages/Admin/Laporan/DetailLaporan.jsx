...
const DetailLaporan = () => {
  ...
  async function handleUpdateStatus(status) {
    if (
      !confirm(
        `Yakin ingin ${status === "resolved" ? "menyetujui" : "menolak"} laporan ini?`,
      )
    )
      return;
    setUpdating(true);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 px-4 py-8">
      ...
      <div className="relative z-10 w-[60vw] bg-white rounded-2xl shadow-xl p-8 flex flex-col gap-6">
        ...
        {/* Tombol Approve / Decline — hanya tampil jika status pending */}
        {laporan?.status === "pending" && (
          <div className="flex gap-3 mt-2">
            <button
              onClick={() => handleUpdateStatus("resolved")}
              disabled={updating}
              className="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-xl transition-colors disabled:opacity-50"
            >
              <FiCheckCircle className="w-5 h-5" />
              {updating ? "Memproses..." : "Approve"}
            </button>
            <button
              onClick={() => handleUpdateStatus("rejected")}
              disabled={updating}
              className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-600 text-white font-bold py-3 rounded-xl transition-colors disabled:opacity-50"
            >
              <FiXCircle className="w-5 h-5" />
              {updating ? "Memproses..." : "Decline"}
            </button>
          </div>
        )}

        {/* Jika sudah resolved/rejected, tampilkan info saja */}
        {laporan?.status !== "pending" && (
